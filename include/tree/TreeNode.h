...
struct TreeNode {
	...
	explicit TreeNode(int v, TreeNode* l = nullptr, TreeNode* r = nullptr, TreeNode* p = nullptr, COLOR c = RED) : val(v), left(l), right(r), parent(p), color(c) {}
...
inline std::vector<TreeNode*> postOrderTravsal(TreeNode* root, void (*func)(TreeNode* pNode, std::vector<TreeNode*>& frame)) {
	auto ret = std::vector<TreeNode*>();
